...
A path of a use case is any graph path that traverses the graph from the initial state $s_1$ to the ($exit$) node. In the use case graph in figure \ref{fig:use-case-graph}, we have the following paths - not counting loops.\bigskip
